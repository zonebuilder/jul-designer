...
/**
	...
	@type	Object
	@name	JUL.Designer.designer.ui
*/
jul.ns('JUL.Designer.designer.ui', {
	tag: 'page',
	children: [
...
/**
	...
	@type	Object
	@name	JUL.Designer.designer.logic
*/
jul.ns('JUL.Designer.designer.logic', {
	'anchor-download': {
		listeners: {
			click: function() {
...
/**
	...
	@type	Object
	@name	JUL.Designer.designer.projectUi
*/
jul.ns('JUL.Designer.designer.projectUi', {
	tag: 'dialog',
	id: 'dialog-project',
	title: 'Project',
	width: 980,
...
/**
	...
	@type	Object
	@name	JUL.Designer.designer.projectLogic
*/
...
/**
	...
	@type	Array
	@name	JUL.Designer.designer.editUi
*/
jul.ns('JUL.Designer.designer.editUi', {
	children: [
...
jul.apply(jul.get('JUL.Designer.designer'), /** @lends JUL.Designer.designer */ {
	/**
	...
	*/
	addComponent: function(sItem) {
		...
		if (this.currentNode) {
			...
			if (this.current.ui) {
				if (JUL.typeOf(this.current.ui) !== 'Array') {
					this.current.ui = [this.current.ui];
					var oFirstWhere = JUL.Designer.getWhere(ample.getElementById('tree-interface').querySelector('xul|treechildren').firstChild);
					oFirstWhere.ref(this.current.ui, 0);
					oFirstWhere.parent = new JUL.Ref({ref: this.current, key: 'ui'});
					bToArray = true;
				}
				/* add a reference to the new component config */
				oWhere = new JUL.Ref({ref: this.current.ui, key: this.current.ui.length,
					parent: new JUL.Ref({ref: this.current, key: 'ui'})});
			}
			else {
				/* add a reference to the new component config */
	...
	*/
	buildTree: function(oParent, oWhere, bOnlyChildren, bSelf) {
		...
		if (JUL.typeOf(oWhere.val()) === 'Array') {
			var aUi = oWhere.val();
			for (var u = 0; u < aUi.length; u++) {
				this.buildTree(oParent, new JUL.Ref({ref: aUi, key: u, parent: oWhere}), bOnlyChildren);
			}
			return;
		}
		if (!oParent) {
	...
	*/
	...
	/**
		Changes the membership of the displayed member collection either to the UI configuration or to the interface logic
	...
	*/
	...
	/**
		Creates an XML document containing the exported XML layout of a config tree.
	...
	*/
	createXml: function(oConfig) {
		...
		var oWidget = aTag.length > 1 && this.xmlDoc.createElementNS && this.xmlNS[aTag[0]] ?
		...
		for (var sItem in JUL.Designer.keySort(oConfig, null, ['type', this.idProperty, this.cssProperty, 'style'])) {
				...
				[this.listenersProperty, this.htmlProperty, this.textProperty, this.tagProperty, this.classProperty, this.parentProperty].indexOf(sItem) < 0)
			{
				...
				if (aAttr.length > 1 && oWidget.setAttributeNS && this.xmlNS[aAttr[0]]) {
	...
	*/
	...
	/**
		...
		This function is given as parameters the current node and an array of path segments to it.
	...
	*/
	findPath: function(oFind, sItem, oRoot, oRootLogic) {
		...
		var bArray = JUL.typeOf(oRoot) === 'Array';
		if (bFunction) {
			if (!bArray && oFind(oRoot, aPath)) { return ''; }
		}
		else {
		...
		if (bArray) {
			for (var u = 0; u < oRoot.length; u++) {
				aPath = [u];
				if (fobj(oRoot[u], 1)) { return aPath.join('.'); } 
			}
			return false;
		}
	...
	*/
	onCopyNode: function(bCut, bRemove) {
		...
		if (this.current.ui && JUL.typeOf(this.current.ui) === 'Array' && this.current.ui.length === 1) {
			this.current.ui = this.current.ui[0];
			var oFirstWhere = JUL.Designer.getWhere(ample.getElementById('tree-interface').querySelector('xul|treechildren').firstChild);
			oFirstWhere.ref(this.current, 'ui');
			delete oFirstWhere.parent;
		}
		this.save(true, true);
		ample.getElementById('tree-interface').reflow();
	},
	...
	*/
	onPasteNode: function() {
		...
		var nStart = typeof this.state.clipboard.pasteStart === 'undefined' ? 0 : this.state.clipboard.pasteStart;
		var nEnd = typeof this.state.clipboard.pasteEnd === 'undefined' ? this.state.clipboard.contentNodes.length - 1 : this.state.clipboard.pasteEnd;
		...
		if (this.currentNode) {
			...
			oWhere = {showWhat: 'ui'};
			oConfig = this.current;
		}
		for (var u = nStart; u <= nEnd; u++) {
			...
			if (!oConfig[oWhere.showWhat] && (!this.current.ui || ample.getElementById('menulist-designer-members-type').getAttribute('Value') === 'Object')) {
				...
				if (JUL.typeOf(oConfig[oWhere.showWhat]) !== 'Array') {
					...
					if (this.currentNode) { ample.getElementById('menulist-designer-members-type').setAttribute('value', 'Array'); }
				}
	...
	*/
	removeTree: function(oNode, bDeleteConfig, bOnlyChildren, bSelf) {
		if (!oNode) {
			...
			if (oNode) {
				while (oNode.lastChild) { this.removeTree(oNode.lastChild, bDeleteConfig, bOnlyChildren); }
			}
			return;
		}
	...
	/**
		Saves a project or a part of it
	...
	*/
	showPath: function(sPath, sWhatChildren) {
		...
		var oRoot = oQuery.get(0);
		var oWhere = null;
		var aPath = sPath.length ? sPath.split('.') : [];
		if (aPath.length && JUL.UI._regExps.uint.test(aPath[0])) {
			oRoot = oRoot.items.item(parseInt(aPath[0]));
			aPath.shift();
		}
		else {
			oRoot = oRoot.items.item(0);
		}
		var oNode = oRoot;
	...
	*/
	toXml: function(oOptions) {
		...
		oOptions = JUL.apply({
			...
			rootWrapper: 'root'
		}, [this.current.parserConfig, this.current.parserConfig._otherProperties || {}, oOptions]);
		...
		if (bArray) {
			var oRootConfig = {};
			oRootConfig[oParser.classProperty] = oParser.useTags ? oParser.defaultClass : oOptions.rootWrapper;
			if (oParser.useTags) { oRootConfig[oParser.tagProperty] = oOptions.rootWrapper; }
			oRootConfig[oParser.parentProperty] = oOptions.xmlDoc;
			oParser.createComponent(oRootConfig);
		}
		oParser.create(oUi, oLogic, bArray ? oOptions.xmlDoc.lastChild : oOptions.xmlDoc);
	...
	undo: function() {
		...
		case 'removeNodes':
			for (i = 0; i < oUndo.ui.length; i++) {
				if (oUndo.wasArray[i]) {
					var oParentWhere = oUndo.where[i].parent;
					oParentWhere.val(oParentWhere.val() || []);
					if (JUL.typeOf(oParentWhere.val()) !== 'Array') {
						oParentWhere.val([oParentWhere.val()]);
					}
					oUndo.where[i].ref(oParentWhere.val());
